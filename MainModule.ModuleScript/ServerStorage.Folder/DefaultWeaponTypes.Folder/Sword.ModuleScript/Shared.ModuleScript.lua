	...
	local WeaponType WeaponType = {
		...
		OnHit = function(Weapon, Part, PartIndex)
			...
			if (not Weapon.DamageCooldown or tick() >= Weapon.DamageCooldown) and Core.CanAttack(Weapon) then
				...
				print("Sword finding damageable", Dmgable, Dmgable and Dmgable.Health, Damageable, Damageable and Core.CanDamage(Weapon.User, Damageable, Part, Weapon))
		...
		Preload = function (Weapon, PreloadArray)
			if Weapon.SlashSound then
				PreloadArray[#PreloadArray + 1] = Weapon.SlashSound
			end
			if Weapon.LungeSound then
				PreloadArray[#PreloadArray + 1] = Weapon.LungeSound
			end
		end,
