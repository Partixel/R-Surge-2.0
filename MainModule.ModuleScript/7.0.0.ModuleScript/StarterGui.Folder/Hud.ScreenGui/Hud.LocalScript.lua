...
while not Assists do
	Assists = LocalPlayer:FindFirstChild("Assists", true)
end
...
local WOs = LocalPlayer:WaitForChild("S2"):WaitForChild("Deaths")

local KOs = LocalPlayer:WaitForChild("S2"):WaitForChild("Kills")
...
LocalPlayer.CharacterAdded:Connect(HealthChanged)
...
function WeaponDeselected(StatObj)
	...
	if StatObj and Core.GetWeapon(StatObj).User ~= LocalPlayer then return end
...
function WeaponSelected(StatObj)
	...
	if Weapon.User ~= LocalPlayer or Weapon.WeaponType ~= Core.WeaponTypes.RaycastGun then return end
...
Core.WeaponTypes.RaycastGun.ClipChanged.Event:Connect(function(StatObj, Ammo)
	...
	if Weapon.User ~= LocalPlayer or not Core.Selected[Weapon.User] or not Core.Selected[Weapon.User][Weapon] then return end
